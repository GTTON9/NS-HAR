...
har_results <- har_results %>%
  mutate(eta = residuals / sqrt(RV_hat))

eta_wide <- har_results %>%
  select(date, Bond_Index, eta) %>%
  pivot_wider(names_from = Bond_Index, values_from = eta) %>%
  drop_na()

#Shoud be run the corrlation martix of the return or the model residuals 
R <- cor(eta_wide %>% select(-date), use = "pairwise.complete.obs")
...
set.seed(42)
n_sim <- nrow(eta_wide)  # simulate same number of days
